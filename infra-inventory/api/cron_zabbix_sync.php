...
try {
    ...
    foreach ($deviceMap as $serial => $device) {
        if (!in_array($serial, $zabbixSerials)) {
            // This device is not in Zabbix anymore, so we decommission it.
            $updateStmt = $pdo->prepare(
                "UPDATE inventory SET status = 'Decommissioned' WHERE id = ?"
            );
            $updateStmt->execute([$device['id']]);

            $logMsg = "Decommissioned device #{$device['id']} (SN: {$serial}) as it was not found in Zabbix.";
            echo $logMsg . "\n";
            writeLog($pdo, 'ZABBIX_SYNC', 'Device Decommissioned', $logMsg);
            $decommissionedCount++;
        }
    }
